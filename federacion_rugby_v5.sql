...
CREATE PROCEDURE InsertarEquipo(IN p_nombre VARCHAR(255), IN p_fechaFundacion YEAR,IN p_dniEntrenador INT)
BEGIN
...
CREATE PROCEDURE ModificarEquipo(IN nombreEquipoActual VARCHAR(50), IN nuevoNombreEquipo VARCHAR(50), IN nuevoAñoFundacion INT, IN nuevoDniEntrenador VARCHAR(20))
BEGIN
    DECLARE existe INT;
    ...
    IF existe > 0 THEN
        ...
        IF nombreDuplicado = 0 THEN
            ...
            ELSE
                SELECT 'Error: El DNI del entrenador no existe en la base de datos' AS Mensaje;
            END IF;
        ELSE
            SELECT 'Error: El nuevo nombre del equipo ya existe en la base de datos' AS Mensaje;
        END IF;
    ELSE
        SELECT 'Error: El equipo no existe en la base de datos' AS Mensaje;
    END IF;
END //

CREATE PROCEDURE GestionarTemporada(IN operacion VARCHAR(10), IN id_temporada INT, IN estado_nuevo VARCHAR(20), IN nombre_temporada VARCHAR(50))  
BEGIN
...
CREATE PROCEDURE InsertarOActualizarEquipo(    nuevoID INT, nuevoNombre VARCHAR(100), nuevoENT_DNI VARCHAR(20))  -- Se agrega ENT_DNI como parámetro
BEGIN
    DECLARE existe INT;
    DECLARE existe_entrenador INT;

    -- Verificamos si el ENT_DNI existe en la tabla entrenador
    SELECT COUNT(*) INTO existe_entrenador
    FROM entrenador
    WHERE ENT_DNI = nuevoENT_DNI;

    -- Si no existe el ENT_DNI en entrenador, mostramos un error y salimos del procedimiento
    IF existe_entrenador = 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error: El ENT_DNI no existe en la tabla entrenador.';
    ELSE

        -- Verificamos si el equipo existe
        SELECT COUNT(*) INTO existe 
        FROM equipo 
        WHERE EQU_ID = nuevoID;

        -- Evaluamos la existencia del equipo
        IF existe > 0 THEN
            -- Si existe, lo actualiza
            UPDATE equipo
            SET EQU_Nombre = nuevoNombre, ENT_DNI = nuevoENT_DNI
            WHERE EQU_ID = nuevoID;

            SELECT CONCAT('Equipo actualizado: ', nuevoNombre) AS Mensaje;
        ELSE
            -- Si no existe, lo inserta
            INSERT INTO equipo (EQU_ID, EQU_Nombre, ENT_DNI)
            VALUES (nuevoID, nuevoNombre, nuevoENT_DNI);
...
CREATE PROCEDURE BorrarEquipoClase(IN EquipoID INT)
BEGIN
    DECLARE NUMJugadores INT DEFAULT 0;

    -- Contar cuántos jugadores están asociados al equipo en la tabla intermedia
    SELECT COUNT(*) INTO NUMJugadores 
    FROM incluyejugador
    WHERE EQU_TEM_ID = EquipoID;

    -- Si hay jugadores, eliminamos primero sus referencias
    IF NUMJugadores > 0 THEN
        DELETE FROM incluyejugador WHERE EQU_TEM_ID = EquipoID;
    END IF;

    -- Eliminar referencias del equipo en incluyeequipo (si existe)
    DELETE FROM incluyeequipo WHERE EQU_ID = EquipoID;

    -- Finalmente, eliminar el equipo de la tabla equipo
    DELETE FROM equipo WHERE EQU_ID = EquipoID;

    -- Mensaje de confirmación
    SELECT CONCAT(
        'Se eliminaron ', NUMJugadores, ' jugadores y el equipo con ID ', EquipoID
    ) AS Mensaje;
END //
...
CREATE PROCEDURE InsertarArbitroSeguro(IN dni_arbitro INT, IN nombre_arbitro VARCHAR(100))
BEGIN
    DECLARE existe INT;
...
CREATE PROCEDURE BorrarJugador(IN jugadorID INT)
BEGIN
    DECLARE existe INT;
...
CREATE PROCEDURE ActualizarNombreEntrenador(IN dni INT, IN nuevoNombre VARCHAR(100))
BEGIN
    DECLARE existe INT;
...
CREATE PROCEDURE InsertarJugadorConLimite(IN jugadorID INT, IN nombreJugador VARCHAR(100), IN equipoID INT)
BEGIN
...
CREATE PROCEDURE InsertarPartidoSeguro(IN idPartido INT, IN idEquipoLocal INT, IN idEquipoVisitante INT, IN fecha DATE)
